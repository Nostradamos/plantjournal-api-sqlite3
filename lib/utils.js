 ...
 * @param  {object} options         - options object which gets passed 'squel');
const Constants = require('./constants');
 to any get function. We
  ...
  _.each(options.where, function(fieldValue, field) {
    ...
    if(_.isInteger(fieldValue) || _.isString(fieldValue)) {
      ...
      let subQuery = squel.select()
        .from(CONSTANTS.tableGenerationParents, 'generation_parents')
 ...
 * @param {object} options      - options which got passed to the get function. For advanced use.
...
* @param {object}  returnObject - Object which will get returned from
                                  pj.{Plant|Plant|Generation|...|Famiy}.get.
                                  Gets mutated.
* @param {object}  options      - options which got passed to the get function.
*/
...
Utils.validateOptions = function validateOptions(fields, options) {
  ...
  _.each(fields, function(field) {
    ...
    if(_.has(options, fieldName)) {
      let optionsField = options[fieldName];
      if(fieldType === 'int' && !_.isInteger(optionsField)) {
        throw new Error('options.' + fieldName + ' has to be an integer');
      } else if(fieldType === 'string' && !_.isString(optionsField)) {

      } else if(fieldType === 'int[]' && (!_.isArray(optionsField) || !_.every(optionsField, _.isInteger))) {
