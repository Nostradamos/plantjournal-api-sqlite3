 ...
 * @param  {string} field - field name. Eg. familyId, familyName, generationId, generationName, generationParent, phenotypeId...
...
Utils.whichTableForField = function whichTableForField(field) {
  ...
  } else if(_.startsWith(field, 'phenotype')) {
    table = Constants.tablePhenotypes;
  } else if(field === 'generationParents') {
...
Utils.addPlantFromRowToReturnObject = function addPlantFromRowToReturnObject(row, returnObject, options, forceAdd) {
  ...
  if(forceAdd === true || (!_.has(returnObject.plants, plantId) && _.has(row, 'plantName'))) {
    let plant = {
      'plantId': row.plantId,
 ...
 * @param {bool}   [forceAdd=false]     - adds to returnObject even if row.generatioName is not set.
 */
Utils.addPhenotypeFromRowToReturnObject = function addPhenotypeFromRowToReturnObject(row, returnObject, options, forceAdd) {
  let phenotypeId = row.phenotypeId;

  if(forceAdd === true || (!_.has(returnObject.phenotypes, phenotypeId) && _.has(row, 'phenotypeName'))) {
    let phenotype = {
...
Utils.leftJoinGenerations = function leftJoinGenerations(query) {
  query.left_join(Constants.tableGenerations, 'generations', 'phenotypes.generationId = generations.generationId');
...
Utils.leftJoinPhenotypes = function leftJoinPhenotypes(query) {
  query.left_join(Constants.tablePhenotypes, 'phenotypes', 'plants.phenotypeId = phenotypes.phenotypeId');
}
