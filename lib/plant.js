...
Plant.create = async function create(options) {
  ...
  // Make sure our paramaters are valid
  ...
  if(_.has(options, 'generationId') && !_.isInteger(options.generationId)) throw new Error('options.generationId has to be an integer');
  if(_.has(options, 'genotypeId') && !_.isInteger(options.genotypeId)) throw new Error('options.genotypeId has to be an integer');
  if(_.has(options, 'plantClonedFrom') && !_.isInteger(options.plantClonedFrom)) throw new Error('options.plantClonedFrom has to be an integer');

  // plantName has to be set
  if(!_.has(options, 'plantName')) throw new Error('options.plantName has to be set');
  if(!_.isString(options.plantName)) throw new Error('options.plantName has to be a string');
