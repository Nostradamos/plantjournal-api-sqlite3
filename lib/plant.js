...
Plant.create = async function create(options) {
  ...
  if(_.isUndefined(generationId) && _.isUndefined(phenotypeId) && _.isNull(plantClonedFrom)) throw new Error('Either options.generationId, options.phenotypeId or options.plantClonedFrom has to be set');
  ...
  // If generationId is set, and phenotypeId is not set, and plantClonedFrom is null,
  // we want to create a new phenotype.
  let phenotype = false;
  if(!_.isUndefined(generationId) && _.isUndefined(phenotypeId) && _.isNull(plantClonedFrom)) {
    ...
    // plantClonedFrom is not null, resolve phenotypeId.
    logger.debug('Plant #create() We need to resolve the phenotypeId from Mother Plant with id:', plantClonedFrom);
    let qMotherPlant = squel.select().from(Constants.tablePlants, 'plants').field(Constants.idFieldPhenotype).where('plants.plantId = ?', plantClonedFrom).limit(1).toString();
    logger.debug('Plant #create() Resolve Mother Plant Phenotype Id Query:', qMotherPlant);
    let row;
    try {
    ...
    logger.debug('Plant #create() Resolved phenotype id of mother plant:', phenotypeId);
  }
