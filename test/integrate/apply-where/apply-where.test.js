  ...
  describe(`#applyWhere`, () => {
      ...
      it(`should throw error if attribute is not in allowedFields`, () => {
        should(
          () => applyWhere(
            ...
            [],
            {where: {
              generationName: 'testGenerationName',
              generationParents: [1,2]
            }})
        ).throw(
          'Illegal attribute or unknown logical operator: generationName');
      });
    });
    ...
    describe(`boolean operators`, () => {
      it(`should do AND conjunction for $and`, () => {
        ...
        q.toString().should.sqlEql(
          `SELECT * FROM test
           WHERE (
             generations.generationId = 'a'
             AND
             generations.generationName = 'b'
           )`);
      });
      ...
      it(`should do AND conjunction for nested $and`, () => {
        ...
        q.toString().should.sqlEql(
          `SELECT * FROM test
           WHERE (
             generations.generationId = 'a'
             AND generations.generationName = 'b'
             AND families.familyId = 'c'
           )`);
      });
      ...
      it(`should do OR conjunction for nested $and`, () => {
        ...
        q.toString().should.sqlEql(
          `SELECT * FROM test
           WHERE (
             generations.generationId = 'a'
             OR generations.generationName = 'b'
             OR families.familyId = 'c'
           )`);
      });
        ...
        q.toString().should.sqlEql(
          `SELECT * FROM test
           WHERE (
             ...
             OR
             (
               generations.generationId = 'c'
               AND
               generations.generationName = 'd'
             )
           )`);
      });
      ...
      it(`should do AND conjunction for dicts if it's not a child of $and/$or..`, () => {
        ...
        q.toString().should.sqlEql(
          `SELECT * FROM test
           WHERE (generations.generationId = 'a' AND families.familyId = 'c')`);
      });
      ...
      it(`should do OR conjunction for arrays if it's not a child of $and/$or..`, () => {
        ...
        q.toString().should.sqlEql(
          `SELECT * FROM test
           WHERE (generations.generationId = 'a' OR families.familyId = 'c')`);
      });
    ...
    describe(`relational operators`, () => {
      ...
      it(`should do 'NOT LIKE' operation for $nlike`, () => {
        ...
        q.toString().should.sqlEql(
          `SELECT * FROM test
           WHERE (generations.generationName NOT LIKE 'foo_')`
        );
      });
      ...
      it(`should be possible to combine multiple operators on same attribute`, () => {
        ...
        q.toString().should.sqlEql(
          `SELECT * FROM test
           WHERE (
             generations.generationName = 'bar'
             AND
             generations.generationName != 'foo'
           )`);
      });
    });
    ...
    describe(`generationParents`, () => {
      it(`should do 'IN $opValue' and 'HAVING COUNT(..) = $opValue.length' for generationParents $eq`, () => {
        applyWhere(
          q,
          ['generationParents'],
          {where: {generationParents: {$eq: [13, 37, 42]}}});
        q.toString().should.sqlEql(
          `SELECT * FROM test
           WHERE (
             generations.generationId IN (
               SELECT generation_parents.generationId
               FROM generation_parents
               WHERE (generation_parents.plantId IN (13, 37, 42))
               GROUP BY generation_parents.generationId
               HAVING (COUNT(generation_parents.plantId) = 3)
             )
           )`);
      });
          ...
          `SELECT * FROM test
           WHERE (
             generations.generationId IN (
               SELECT generation_parents.generationId
               FROM generation_parents
               WHERE (generation_parents.plantId IN (42, 43))
               GROUP BY generation_parents.generationId
               HAVING (COUNT(generation_parents.plantId) = 2)
             )
           )`);
      });
      ...
      it(`should do 'NOT IN' and 'HAVINC COUNT(..) >= $opValue.length' for generationParents $neq`, () => {
        applyWhere(
          q,
          ['generationParents'],
          {where: {generationParents: {$neq: [13, 37, 42]}}});
             ...
             generations.generationId IN (
               ...
               WHERE (
                 generations.generationId IN (
                   ...
                   SELECT generation_parents.generationId
                   FROM generation_parents
                   WHERE (generations.generationId NOT IN (13, 37, 42)))
                 )
               )
             )`);
      });
      ...
      it(`should do 'NOT IN' for generationParents $neq when operator value is a single integer`, () => {
        applyWhere(
          q, ['generationParents'], {where: {generationParents: {$neq: 42}}});
        q.toString().should.sqlEql(
          `SELECT * FROM test
           WHERE (
             generations.generationId IN (
               SELECT generation_parents.generationId
               FROM generation_parents
               WHERE (generations.generationId
               IN (
                 SELECT generation_parents.generationId
                 FROM generation_parents
                 GROUP BY generation_parents.generationId
                 HAVING (COUNT(generation_parents.plantId) != 1)
                 UNION
                 SELECT generation_parents.generationId
                 FROM generation_parents
                 WHERE (generations.generationId NOT IN (42)))
               )
             )
           )`);
      });
      ...
      it(`should do 'LIKE' for generationParents $like`, () => {
        applyWhere(
          q,
          ['generationParents'],
          {where: {generationParents: {$like: '13_7'}}});
          ...
          `SELECT * FROM test
           WHERE (
             generations.generationId IN (
               SELECT generation_parents.generationId
               FROM generation_parents
               WHERE (generation_parents.plantId LIKE '13_7')
             )
           )`);
      });
      ...
      it(`should do 'NOT LIKE' for generationParents $nlike`, () => {
        applyWhere(
          q,
          ['generationParents'],
          {where: {generationParents: {$nlike: '13_7'}}});
          ...
          `SELECT * FROM test
           WHERE (
             generations.generationId IN (
               SELECT generation_parents.generationId
               FROM generation_parents
               WHERE (generation_parents.plantId NOT LIKE '13_7')
             )
           )`);
      });
      ...
      it(`should do '>' for generationParents $gt`, () => {
        applyWhere(
          q, ['generationParents'], {where: {generationParents: {$gt: 42}}});
        q.toString().should.sqlEql(
          `SELECT * FROM test
           WHERE (
             generations.generationId IN (
               SELECT generation_parents.generationId
               FROM generation_parents
               WHERE (generation_parents.plantId > 42)
             )
           )`);
      });
      ...
      it(`should do '>=' for generationParents $gte`, () => {
        applyWhere(
          q, ['generationParents'], {where: {generationParents: {$gte: 42}}});
        q.toString().should.sqlEql(
          `SELECT * FROM test
           WHERE (
             generations.generationId IN (
      ...
      it(`should do '<' for generationParents $lt`, () => {
        applyWhere(
          q, ['generationParents'], {where: {generationParents: {$lt: 42}}});
        q.toString().should.sqlEql(
          `SELECT * FROM test
           WHERE (
             generations.generationId IN (
      ...
      it(`should do '<=' for generationParents $lte`, () => {
        applyWhere(
          q, ['generationParents'], {where: {generationParents: {$lte: 42}}});
        q.toString().should.sqlEql(
          `SELECT * FROM test
           WHERE (
             generations.generationId IN (
      ...
      it(`should do 'NOT IN' for generationParents $nin`, () => {
        applyWhere(
          q,
          ['generationParents'],
          {where: {generationParents: {$nin: [42, 43]}}});
        q.toString().should.sqlEql(
          `SELECT * FROM test
           WHERE (
             generations.generationId IN (
               SELECT generation_parents.generationId
               FROM generation_parents
               WHERE (generation_parents.plantId NOT IN (42, 43))
             )
           )`);
      });
      ...
      it(`should do 'IN $opV' and 'HAVING COUNT >= $opV.length' for $contains operator`, () => {
        applyWhere(
          q,
          ['generationParents'],
          {where: {generationParents: {$contains: [13, 37, 42]}}});
        q.toString().should.sqlEql(
          `SELECT * FROM test
           WHERE (
             generations.generationId IN (
      ...
      it(`should do 'NOT IN $opV' and for $ncontains operator`, () => {
        applyWhere(
          q,
          ['generationParents'],
          {where: {generationParents: {$ncontains: [13, 37, 42]}}});
        q.toString().should.sqlEql(
          `SELECT * FROM test
           WHERE (
             generations.generationId IN (
               ...
               FROM generation_parents
               WHERE (
                 generation_parents.generationId NOT IN (
                   SELECT generation_parents.generationId
                   FROM generation_parents
                   WHERE (generation_parents.plantId IN (13, 37, 42))
                  GROUP BY generation_parents.generationId
    ...
    describe(`journalValue`, () => {
      it(`should set WHERE journals.journalValue = '"foo"' if attr is just journalValue`, () => {
        ...
        q.toString().should.sqlEql(
          `SELECT * FROM test
           WHERE (journals.journalValue = '"foo"')`);
      });

      it(`should set WHERE JSON_EXTRACT(journals.journalValue, "$.foo.bar[2]") = "foo" if attr is journalValue.foo.bar[2]`, () => {
        applyWhere(
          q, ['journalValue'], {where: {'journalValue.foo.bar[2]': 'foo'}});
        q.toString().should.sqlEql(
          `SELECT * FROM test
           WHERE (
             JSON_EXTRACT(journals.journalValue, '$.foo.bar[2]') = 'foo'
           )`);
      });
    });
    ...
    describe(`old tests`, () => {
      ...
      it(`should set WHERE (translated)field = fieldValue if options.where[field] = fieldValue is an integer and correctly translate field to database.databasefield`, () => {
        ...
        q.toString().should.eql(
          `SELECT * FROM test WHERE (families.familyId = 42)`);
      });
      ...
      it(`should set WHERE (translated)field = "fieldValue" if options.where[field] = fieldValue is a string`, () => {
        applyWhere(
          q,
          ['generationName'],
          {where: {generationName: 'testGenerationName'}});
        q.toString().should.sqlEql(
          `SELECT * FROM test
           WHERE (generations.generationName = 'testGenerationName')`);
      });
      ...
      it(`should do nothing if options.where key is valid but value is something we don't know how to handle (for field !== generationParents)`, () => {
        applyWhere(
          q, ['generationName'], {where: {generationName: () => {}}});
        q.toString().should.eql(`SELECT * FROM test`);
      });

      it(`should do nothing if options.where key is valid but value is something we don't know how to handle (for field === generationParents)`, () => {
        applyWhere(
          q, ['generationParents'], {where: {generationParents: () =>{}}});
        q.toString().should.eql(`SELECT * FROM test`);
      });
    });
  });
